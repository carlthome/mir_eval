...
def _get_reference_beat_variations(reference_beats):
    ...
    double_reference_beats = np.interp(interpolated_indices, original_indices,
                                       reference_beats)
    # Return metric variations:
    # True, off-beat, double tempo, half tempo odd, and half tempo even
    return (reference_beats, double_reference_beats[1::2],
            double_reference_beats, reference_beats[::2],
            reference_beats[1::2])
...
def f_measure(reference_beats, estimated_beats, f_measure_threshold=0.07):
    ...
    matching = util.match_events(reference_beats, estimated_beats,
                                 f_measure_threshold)
...
def p_score(reference_beats, estimated_beats, p_score_threshold=0.2):
    ...
    end_point = np.int64(
        np.ceil(np.max([np.max(estimated_beats),
    ...
    win_size = int(
        np.round(p_score_threshold * np.median(annotation_intervals)))
...
def continuity(reference_beats,
    ...
    for reference_beats in _get_reference_beat_variations(reference_beats):
        ...
        n_annotations = np.max(
            [reference_beats.shape[0], estimated_beats.shape[0]])
            ...
            if used_annotations[nearest] == 0:
                ...
                # This beat/annotation is not the first
                    ...
                    reference_interval = (reference_beats[nearest] -
                                          reference_beats[nearest - 1])
                    period = np.abs(1 -
                                    estimated_interval / reference_interval)
                    if phase < continuity_phase_threshold and \
                       period < continuity_period_threshold:
                        # Set this annotation as used
                        used_annotations[nearest] = 1
        ...
        total_accuracy = np.sum(beat_successes) / (1.0 *
                                                   beat_successes.shape[0])
        total_accuracies.append(total_accuracy)
    # Grab accuracy scores
    return (continuous_accuracies[0], total_accuracies[0],
            np.max(continuous_accuracies), np.max(total_accuracies))
...
def evaluate(reference_beats, estimated_beats, **kwargs):
    ...
    # Goto
    scores['Goto'] = util.filter_kwargs(goto, reference_beats, estimated_beats,
                                        **kwargs)
    ...
    (scores['Correct Metric Level Continuous'],
     ...
     scores['Any Metric Level Total']) = util.filter_kwargs(
         continuity, reference_beats, estimated_beats, **kwargs)
