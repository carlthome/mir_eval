...
def test_clicks():
    ...
    for times in [np.array([1.]), np.arange(10) * 1.]:
        for fs in [8000, 44100]:
            ...
            click_signal = mir_eval.sonify.clicks(times,
                                                  fs,
                                                  click=np.zeros(1000))
...
def test_time_frequency():
    ...
    for fs in [8000, 44100]:
        ...
        signal = mir_eval.sonify.time_frequency(np.random.standard_normal(
            (100, 1000)),
                                                np.arange(1, 101),
...
def test_chroma():
    for fs in [8000, 44100]:
        signal = mir_eval.sonify.chroma(np.random.standard_normal((12, 1000)),
                                        np.linspace(0, 10, 1000), fs)
        assert len(signal) == 10 * fs
        signal = mir_eval.sonify.chroma(np.random.standard_normal((12, 1000)),
                                        np.linspace(0, 10, 1000),
...
def test_pitch_contour():
    ...
    x = mir_eval.sonify.pitch_contour(times,
                                      freqs,
                                      fs,
                                      length=fs * 7,
                                      amplitudes=amps)
