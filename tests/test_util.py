...
def test_interpolate_intervals():
    ...
    assert (util.interpolate_intervals(intervals, labels, time_points,
                                       'N') == expected_ans)
...
def test_merge_labeled_intervals():
    ...
    x_intvs = np.array([[0.0, 0.44], [0.44, 2.537], [2.537, 4.511],
                        [4.511, 6.409]])
    x_labels = ['A', 'B', 'C', 'D']
    y_intvs = np.array([[0.0, 0.464], [0.464, 2.415], [2.415, 4.737],
                        [4.737, 6.409]])
    y_labels = [0, 1, 2, 3]
    expected_intvs = [[0.0, 0.44], [0.44, 0.464], [0.464, 2.415],
                      [2.415, 2.537], [2.537, 4.511], [4.511, 4.737],
                      [4.737, 6.409]]
...
def test_outer_distance_mod_n():
    ref = [1., 2., 3.]
    est = [1.1, 6., 1.9, 5., 10.]
    expected = np.array([[0.1, 5., 0.9, 4., 3.], [0.9, 4., 0.1, 3., 4.],
                         [1.9, 3., 1.1, 2., 5.]])
...
def test_validate_intervals():
    ...
    nose.tools.assert_raises(ValueError, mir_eval.util.validate_intervals,
                             np.array([[1.], [2.5], [5.]]))
...
def test_validate_events():
    ...
    nose.tools.assert_raises(ValueError, mir_eval.util.validate_events,
                             np.array([100., 100000.]))
...
def test_validate_frequencies():
    ...
    nose.tools.assert_raises(ValueError, mir_eval.util.validate_frequencies,
